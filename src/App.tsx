...
const SCENARIOS = [
  {
    ...
    context:
      'Bạn Thỏ đang có một chiếc ô tô màu đỏ rất đẹp. Bé muốn mượn bạn Thỏ chơi một lát, bé sẽ nói thế nào nhỉ?',
    rules: {
      action: [
        'mượn',
        'cho mình',
    ...
    hints: {
      hasImpolite:
        'Ôi, bé nói như vậy nghe giống ra lệnh quá. Mình xin mượn nhẹ nhàng hơn nhé.',
      missingAction:
        "Bé muốn mượn đồ chơi thì bé dùng từ 'mượn' hoặc 'chơi chung' nhé.",
      missingPolite:
        "Câu của bé hơi cộc lốc rồi. Bé thêm chữ 'nhé' hoặc 'được không' ở cuối câu cho lịch sự nha.",
      tooShort:
        "Bé nói câu dài thêm một chút nữa nhé, ví dụ: 'Bạn Thỏ cho mình mượn nhé!'",
    },
    evaluation:
      "Tuyệt vời! Bé xin mượn rất lịch sự! Để câu nói hay hơn nữa, lần sau bé có thể khen: 'Ô tô của bạn đẹp quá, cho mình mượn nhé'. Bạn Thỏ sẽ rất thích đấy!",
  },
  {
    id: 2,
    ...
    context:
      'Ôi thôi, bé lỡ tay làm rơi hộp bút của bạn Gấu xuống đất mất rồi. Bé hãy nói một câu để bạn Gấu không buồn nhé!',
    rules: {
    ...
    hints: {
      hasImpolite:
        'Bé ơi, mình làm rơi đồ thì không được nói vậy đâu. Bé hãy nói lời xin lỗi chân thành nhé.',
      missingAction:
        "Khi làm rơi đồ của bạn, bé phải nói từ gì nhỉ? Bé nhớ nói lời 'xin lỗi' nhé.",
      missingPolite:
        "Bé xin lỗi ngoan rồi, nhưng thêm từ 'mình lỡ tay' hoặc chữ 'nhé' thì bạn Gấu sẽ hết buồn liền.",
      tooShort:
        "Bé nói câu dài thêm một chút nhé, ví dụ: 'Mình xin lỗi bạn Gấu nhé'.",
    },
    evaluation:
      "Bé ngoan lắm! Bé biết nhận lỗi là một em bé dũng cảm. Lần sau bé có thể nói thêm 'Mình sẽ nhặt lên giúp bạn nhé' để thể hiện sự quan tâm nha!",
  },
  {
    id: 3,
    title: 'Nhận quà tặng',
    ...
    context:
      'Hôm nay là sinh nhật bé, bạn Sóc mang đến tặng bé một hộp quà rất to. Bé sẽ nói gì với bạn Sóc?',
    rules: {
    ...
    aiCharacter: 'Bạn Sóc',
    successReply:
      'Không có gì! Mình mong là bạn sẽ thích món quà này. Chúc mừng sinh nhật bạn nha!',
    hints: {
      hasImpolite:
        'Ôi, khi được nhận quà mình không nên nói vậy đâu. Bé hãy vui vẻ nói lời cảm ơn nha.',
      missingAction: "Khi được bạn tặng quà, bé nhớ nói lời 'cảm ơn' nhé.",
      missingPolite:
        "Bé xưng hô thêm 'mình cảm ơn bạn' thì câu nói sẽ tình cảm hơn rất nhiều đó.",
      tooShort:
        "Bé nói dài thêm một chút nhé, ví dụ: 'Mình cảm ơn bạn Sóc nhiều lắm'.",
    },
    evaluation:
      "Bé nói lời cảm ơn to và rõ ràng lắm! Lần sau, bé có thể ôm bạn một cái và nói 'Món quà đẹp quá, mình thích lắm'. Chắc chắn bạn Sóc sẽ rất vui!",
  },
  {
    id: 4,
    ...
    context:
      'Bé đang xếp lâu đài cát rất vui, bé thấy bạn Mèo đang đứng nhìn một mình. Bé hãy rủ bạn Mèo vào chơi cùng nhé!',
    rules: {
      action: [
        'chơi cùng',
        'chơi chung',
    ...
    aiCharacter: 'Bạn Mèo',
    successReply:
      'Meo meo, thích quá! Mình vào xếp lâu đài cát cùng bạn ngay đây!',
    hints: {
      hasImpolite:
        'Bé gọi bạn nhẹ nhàng thôi nhé, mình không nên dùng từ ra lệnh như vậy đâu.',
      missingAction: "Bé hãy vẫy tay và rủ bạn Mèo 'vào chơi cùng' nhé.",
      missingPolite:
        "Bé rủ bạn nhớ thêm chữ 'nhé' hoặc 'nào' ở cuối câu cho thân thiện nha.",
      tooShort:
        "Bé nói câu rủ bạn dài hơn một chút nhé, ví dụ: 'Bạn Mèo vào chơi cùng mình nhé'.",
    },
    evaluation:
      "Bé rủ bạn rất thân thiện! Để lời mời hấp dẫn hơn, bé có thể vừa vẫy tay vừa nói 'Bạn vào đây chơi xếp lâu đài chung cho vui nhé'.",
  },
];
...
export default function App() {
  ...
  useEffect(() => {
    // SỬA LỖI NGHIÊM TRỌNG: Chỉ khởi tạo Audio 1 lần duy nhất, không tạo mới khi đổi Phase (trạng thái)
      ...
      recognitionRef.current.onerror = (event) => {
        ...
        if (phase === 'listening') {
          ...
          setCurrentCharacterText(
            'Cô chưa nghe rõ bé nói gì. Bé nhấn lại Micro và nói to lên nhé!'
          );
        }
      };
  ...
  const speakText = (text, isCharacter = false, callback = null) => {
    ...
    // Xóa hoàn toàn thuật toán băm nhỏ câu.
    // Google TTS hỗ trợ 200 ký tự. Tất cả các câu trong game đều dưới 160 ký tự,
    // nên việc gửi nguyên câu sẽ giữ được ngữ điệu tự nhiên 100% và chặn đứng lỗi spam.
    ...
    if (playPromise !== undefined) {
      playPromise.catch((error) => {
        // CHÌA KHÓA SỬA LỖI: Bỏ qua lỗi AbortError.
        // Khi ngắt tiếng chủ động, trình duyệt sẽ ném ra lỗi AbortError.
        // Nếu bắt lỗi này mà gọi fallback, nó sẽ sinh ra giọng robot ở câu đầu tiên!
        if (error.name === 'AbortError') return;

        console.warn('Trình duyệt chặn Autoplay, chuyển giọng mặc định', error);
        audio.onerror();
      });
    }
  };
  ...
  const handleStartGame = async () => {
    // Mở khóa âm thanh ngay khi người dùng bấm nút vật lý (bắt buộc)
    if (currentAudioRef.current) {
    ...
    try {
      ...
      alert(
        'Bé cần cho phép trình duyệt sử dụng Micro để có thể nói chuyện với các bạn động vật nhé!'
      );
    }
  };
  ...
  const handleAnalyzeSpeech = (text) => {
    ...
    let errorType = null;
    const hasImpolite = scenario.impolite.some((word) =>
      lowercaseText.includes(word)
    );
    const hasAction = scenario.rules.action.some((word) =>
      lowercaseText.includes(word)
    );
    const hasPolite = scenario.rules.polite.some((word) =>
      lowercaseText.includes(word)
    );
    ...
    setTimeout(() => {
      if (!errorType) {
        ...
        setSpeakerData({ name: 'Cô giáo AI', icon: '👩‍🏫', isAI: true });
        const hintMsg =
          scenario.hints[errorType] ||
          'Bé thử nói lại một lần nữa rõ ràng hơn nhé.';
  ...
  const nextScenario = () => {
    if (currentStep < SCENARIOS.length - 1) {
      ...
      setSpeakerData({ name: 'Cô giáo AI', icon: '👩‍🏫', isAI: true });
      const finishText =
        'Tuyệt vời quá! Bé đã hoàn thành xuất sắc các tình huống giao tiếp. Bé là một người bạn vô cùng lịch sự và đáng yêu!';
    ...
    return (
      <div className="min-h-screen bg-red-50 flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-gray-700">
          Trò chơi cần tính năng Nhận diện giọng nói. Vui lòng mở ứng dụng bằng
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-100 to-indigo-200 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-8 text-center border-8 border-white/50 relative overflow-hidden">
          ...
          <p className="text-gray-600 mb-8 text-lg font-medium leading-relaxed relative z-10">
            Cùng học cách giao tiếp lịch sự và bày tỏ cảm xúc với các bạn động
            vật nhé!
          </p>
          <button
  ...
  return (
    <div
      className={`min-h-screen p-2 md:p-6 font-sans flex flex-col transition-colors duration-1000 bg-gradient-to-br ${scenario.bgGradient}`}
    >
      {/* Header Bar */}
      ...
      <div className="flex-1 w-full max-w-4xl mx-auto flex flex-col items-center justify-center relative">
        {phase === 'finished' ? (
          ...
          <div className="w-full flex flex-col items-center">
            {/* Speech Bubble */}
            <div
              className={`relative max-w-2xl w-full bg-white rounded-3xl p-6 md:p-8 shadow-xl border-4 mb-8 transform transition-all duration-300 ${
                speakerData.isAI
                  ? 'border-indigo-100 translate-y-0 opacity-100'
                  : 'border-purple-200 scale-95 opacity-80'
              }`}
            >
              ...
              <p
                className={`text-xl md:text-2xl leading-relaxed text-center font-medium ${
                  phase === 'listening' || phase === 'thinking'
                    ? 'text-gray-400 animate-pulse text-4xl'
                    : 'text-gray-800'
                }`}
              >
                {currentCharacterText}
            ...
            <div className="relative mt-4">
              <div
                className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full blur-3xl opacity-50 ${
                  phase === 'evaluation' ? 'bg-yellow-300' : 'bg-white'
                }`}
              ></div>

              <div
                className={`text-[150px] md:text-[200px] leading-none drop-shadow-2xl relative z-10 transition-transform duration-500 ${
                  speakerData.isAI &&
                  phase !== 'listening' &&
                  phase !== 'thinking'
                    ? 'scale-110 animate-[bounce_3s_ease-in-out_infinite]'
                    : 'scale-100 opacity-90'
                }`}
              >
                {phase === 'evaluation' || phase === 'hint' || phase === 'intro'
                  ? '👩‍🏫'
                  : scenario.image}
              </div>
          ...
          <div className="w-full max-w-xl flex flex-col items-center">
            {phase === 'evaluation' || (phase === 'success' && score > 0) ? (
              ...
              <>
                ...
                >
                  {isListening
                    ? 'Nhấn vào micro để hoàn thành câu nói'
                    : 'Nhấn Micro để nói chuyện với bạn nhé!'}
                </p>
                ...
                >
                  ...
                  <div
                    className={`relative z-10 w-24 h-24 rounded-full flex items-center justify-center text-white shadow-2xl transition-all duration-300 ${
                      isListening
